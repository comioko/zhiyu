...
const CreatePage = () => {
  const { user, tokens, refresh } = useAuth();
  ...
  const getLatestAccessToken = async (): Promise<string> => {
    if (!tokens?.accessToken) {
      throw new Error("登录状态已失效，请重新登录后再发布");
    }
    await refresh();
    try {
      const raw = localStorage.getItem("zhiyu_auth_tokens");
      if (raw) {
  ...
  const ensureDraft = async (): Promise<string> => {
    ...
    const resp = await knowpostService.createDraft(accessToken);
  ...
  const handleSelectImages = async (files: FileList | null) => {
    ...
    try {
      console.log('[CreatePage] 开始处理图片上传，文件数量:', files.length);
      const id = await ensureDraft();
      const accessToken = await getLatestAccessToken();
      console.log('[CreatePage] 草稿ID:', id);
      ...
      for (const f of arr) {
        console.log('[CreatePage] 处理文件:', f.name);
        ...
        console.log('[CreatePage] 请求预签名 URL，参数:', { scene: "knowpost_image", postId: id, contentType, ext });
        
        let presign;
        try {
          presign = await knowpostService.presign({
            scene: "knowpost_image",
            postId: id,
            contentType,
            ext
          }, accessToken);
          console.log('[CreatePage] ✅ 获取预签名成功:', presign);
          console.log('[CreatePage] 预签名 URL (前200字符):', presign.putUrl.substring(0, 200));
          console.log('[CreatePage] 预签名 Headers:', presign.headers);
        } catch (presignError) {
          console.error('[CreatePage] ❌ 获取预签名失败:', presignError);
          throw new Error('获取预签名 URL 失败：' + (presignError instanceof Error ? presignError.message : '未知错误'));
        }
        
        console.log('[CreatePage] 开始上传文件到 OSS...');
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      // 5) 发布
      await knowpostService.publish(id, accessToken);
      setMessage("发布成功 ✅");
    } catch (err) {
  ...
  const handleToggleAiSummary = async () => {
    ...
    if (!aiSummaryEnabled) {
      ...
      try {
        const accessToken = await getLatestAccessToken();
        const resp = await knowpostService.suggestDescription(content, accessToken);
